...
func ingestWeatherHandler(c *Client, batch []BatchUnitData) bool {
	...
	log.Debugf("action: ingest_weather_handler | result: success | client_id: %v  | msg: Batch de %v weathers.", c.config.ID, len(batch))
	return true
}
...
func ingestStationHandler(c *Client, batch []BatchUnitData) bool {
	...
	log.Debugf("action: ingest_station_handler | result: success | client_id: %v  | msg: Batch de %v stations.", c.config.ID, len(batch))
	return true
}
...
func ingestTripsHandler(c *Client, batch []BatchUnitData) bool {
	...
	log.Debugf("action: ingest_trip_handler | result: success | client_id: %v  | msg: Batch de %v trips.", c.config.ID, len(batch))
	return true
}
